$(function () {
    ...
    $('.sentence').bind('keydown keyup keypress change',function() {
        ...
        var minus_num = Number($(this).closest('.input').nextAll().find('.movie_time').first().val());
        var plus_num = Number(movieTime);
        calc_movie_time_sum(true, minus_num, plus_num);
        $(this).closest('.input').nextAll().find('.movie_time').first().val(movieTime);
        // 発話フラグが立っている場合
