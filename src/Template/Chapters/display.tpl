...
<div id="phrases">
    ...
    {foreach from=$chapter['phrases'] item=_phrase name=phraseLoop2}
        <div class="speak" v-show="num >={$smarty.foreach.phraseLoop2.iteration}" ref="speak_{$smarty.foreach.phraseLoop2.iteration}">
            {$this->element('phrase/_display', ['character'=>$_phrase->character,'sentence'=>$_phrase->sentence, 'speakerName'=>$_phrase->speaker_name, 'speakerColor'=>$_phrase->speaker_color, 'noCssFlg' => true])}
        </div>
    {/foreach}
