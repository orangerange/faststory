 ...
 */
class CharacterPartsTable extends Table {
	...
	public function initialize(array $config) {
		...
		$this->setTable('character_parts');
		$this->belongsTo('Characters', [
			'foreignKey' => 'character_id'
		]);
	}

	public function findListByCharacterId($characterId) {
		return $this->find('list', ['keyField'=>'parts_category_no', 'valueField'=>'parts_no'])->where(['character_id' => $characterId])->order(['parts_category_no']);
	}

	public function deleteByCharacterId($characterId) {
		if (!$this->deleteAll(['character_id'=>$characterId])) {
			return false;
		}
		return true;
	}
